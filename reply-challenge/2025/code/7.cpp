...
// Modified to use a fixed depth of 3
double simulate_future(int turn_num, int budget, std::vector<Resource> active_resources, 
                      const std::vector<ResourceInfo>& resources, const std::vector<Turn>& turns, int depth = 3) {
    ...
    int max_resources_to_consider = (depth > 2) ? 3 : resources.size();
...
EvaluationResult evaluate_resource(
    ...
    // Modified to use fixed depth of 3
    double future_profit = simulate_future(turn_num + 1, sim_budget, next_active, all_resources, turns, 3);
...
// Fixed simulation depth to always be 3
...
int main() {
    ...
    std::cerr << "Using fixed simulation depth of 3" << std::endl;
