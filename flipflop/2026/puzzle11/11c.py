...
def solve():
    ...
    while queue:
        ...
        if lastd != d:
            lastd = d
            # print(f"day {d} size {len(tree)}")
            # pprint(tree, stems)
            for et in range(lentrees):
...
stemslist = sorted(list(stems), key=lambda s: (s[0], -s[1]))
...
index = 0
for i, (x, y) in enumerate(stemslist):
    if i > 0 and x == stemslist[i - 1][0]:
        assert stemslist[i - 1][1] > y
        continue
    for (what, t), val in dnas.items():
        if t == tree[(x, y)][1]:
            newdnas[(what, index)] = val
    newtree[(x, 0)] = ("00", index)
    newqueue.append((x, 0, 1, index))
    newstems.add((x, 0))
    index += 1
